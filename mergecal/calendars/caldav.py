...
def clean_caldav_description(description: str) -> str:
...
def is_caldav_url(url: str) -> bool:
    # Parse the URL
...
def extract_caldav_group_name(url: str) -> str | None:
    # Parse the URL
    parsed_url = urlparse(url)
...
def create_calendar_from_caldav_api_response(events: list[dict[str, Any]]) -> Ical:
    cal = Ical()
    cal.add("prodid", "-//MergeCal//CalDav Calendar//EN")
    cal.add("version", "2.0")
    cal.add("x-wr-calname", "CalDav Events")
    ...
    for event in events:
        ...
        e.add("uid", f"caldav-{event['id']}@mergecal.org")  # Use Meetup event ID as UID
        ...
        # Description and URL
        description = clean_caldav_description(event["description"])
        ...
        # Add custom CalDav-specific properties
        e.add("x-caldav-event-id", event["id"])
        e.add("x-caldav-group-id", str(event["group"]["id"]))
        e.add("x-caldav-rsvp-limit", str(event.get("rsvp_limit", "")))
        e.add("x-caldav-yes-rsvp-count", str(event.get("yes_rsvp_count", 0)))
...
def fetch_and_create_caldav_calendar(caldav_url: str) -> Ical | None:
    try:
        is_caldav_url(caldav_url)
        caldav_group_name = extract_caldav_group_name(caldav_url)
        caldav_api_url = f"https://api.meetup.com/{caldav_group_name}/events"
        response = requests.get(caldav_api_url, timeout=10)
        response.raise_for_status()
        caldav_events = response.json()
        return create_calendar_from_caldav_api_response(
            caldav_events,
        )
    except requests.exceptions.HTTPError as e:
        logger.warning(
            "CalDav: Unable to fetch calendar from URL %s. HTTP error: %s",
            caldav_url,
            e,
        )
        return None
    except Exception:
        logger.exception("CalDav: Unexpected error with URL %s", caldav_url)
